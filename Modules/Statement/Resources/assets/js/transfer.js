...
initDatatable = function () {
  if ($('#data-datatable').length > 0) {
    oTable = $('#data-datatable').DataTable({
      ...
      stateSave: true,
      ajax: {
        url: '/admin/statement/transfer/datatable_ajax',
        data: function (d) {
      ...
      columns: [
        ...
        { data: 'step', orderable: true },
        { data: 'status', orderable: true },
        { data: 'msg', orderable: true },
...
setStatus = function (id, status) {
  ...
  $.ajax({
    url: '/admin/statement/transfer/set_status',
    type: 'POST',
    data: {
      id: id,
      status: status,
  ...
  bootbox.confirm({
    ...
    callback: function (result) {
      if (result) {
        ...
        $.ajax({
          url: '/admin/statement/transfer/set_delete',
          type: 'POST',
          data: {
            id: id,
            _token: _token,
