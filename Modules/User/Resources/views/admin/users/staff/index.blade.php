                ...
                <div class="ps-3">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0 p-0">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Branch Staff</li>
                        </ol>
                    </nav>
                </div>
                <div class="ms-auto">
                    ...
                    <div class="card bg-primary text-white">
                        <div class="card-body">
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    <h4 class="mb-0">{{ $stats['total_branch_staff'] ?? 0 }}</h4>
                                    <p class="mb-0">Total Branch Staff</p>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="staff-datatable" class="table table-bordered table-striped text-nowrap w-100">
                            <thead>
                                <tr>
    ...
    <script>
        $(document).ready(function() {
            $('#staff-datatable').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: '{{ route("admin.user.staff.datatable_ajax") }}',
                    type: 'GET'
                },
                columns: [
        ...
        function deleteStaffUser(id) {
            ...
            $.ajax({
                url: '{{ route('admin.user.staff.set_delete') }}',
                type: 'POST',
                data: {
                    id: id,
                    _token: '{{ csrf_token() }}'
                ...
                success: function(resp) {
                    if (resp.success) {
                        ...
                        $('#staff-datatable').DataTable().ajax.reload(null, false);
                    } else {
                        noti('error', resp.msg);
                    }
                },
        ...
        function toggleStaffUserStatus(id, status) {
            $.ajax({
                url: '{{ route('admin.user.staff.set_status') }}',
                type: 'POST',
                data: {
                    id: id,
                    status: status,
