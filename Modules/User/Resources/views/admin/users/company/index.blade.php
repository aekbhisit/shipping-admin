                ...
                <div class="ps-3">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0 p-0">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Company Administrators</li>
                        </ol>
                    </nav>
                </div>
                <div class="ms-auto">
                    <div class="btn-group">
                        <a href="{{ route('admin.user.company.add') }}">
                    ...
                    <div class="card bg-primary text-white">
                        <div class="card-body">
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    <h4 class="mb-0">{{ $stats['total_company_admins'] ?? 0 }}</h4>
                                    <p class="mb-0">Total Company Admins</p>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="company-admin-datatable" class="table table-bordered table-striped text-nowrap w-100">
                            <thead>
                                <tr>
...
@section('scripts')
    <script>
        $(document).ready(function() {
            $('#company-admin-datatable').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: '{{ route("admin.user.company.datatable_ajax") }}',
                    type: 'GET'
                },
                columns: [
        ...
        function deleteCompanyUser(id) {
            ...
            $.ajax({
                url: '{{ route('admin.user.company.set_delete') }}',
                type: 'POST',
                data: {
                    id: id,
                    _token: '{{ csrf_token() }}'
                ...
                success: function(resp) {
                    if (resp.success) {
                        ...
                        $('#company-admin-datatable').DataTable().ajax.reload(null, false);
                    } else {
                        noti('error', resp.msg);
                    }
                },
        ...
        function toggleCompanyUserStatus(id, status) {
            $.ajax({
                url: '{{ route('admin.user.company.set_status') }}',
                type: 'POST',
                data: {
                    id: id,
                    status: status,
