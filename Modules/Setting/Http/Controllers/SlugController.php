...
class SlugController extends Controller
{
    ...
    {
        $o_slugs  =  Slugs::where('lang', app()->getLocale())->where('slug', $slug)->first();
        $lang_switch = [];
        if (!empty($o_slugs->lang_slug)) {
            $param = (!empty($o_slugs->param)) ? '?' . http_build_query(json_decode($o_slugs->param, 1)) : '';
            $lang_switch[$o_slugs->lang] = '/' . $o_slugs->slug . $param;
        }
    ...
    {
        $slug = Slugs::where('id', $id)->first();
        $data = [];
        if (!empty($slug)) {
            $param = (!empty($slug->param)) ? '?' . http_build_query(json_decode($slug->param, 1)) : '';
            $data['url'][$slug->lang] = '/' . $slug->slug . $param;
        }
        return $data;
    }
    ...
    {
        $data = [];
        $slug = Slugs::where([['module', $module], ['method', $method], ['data_id', $data_id]])->first();
        if (!empty($slug)) {
            $param = (!empty($slug->param)) ? '?' . http_build_query(json_decode($slug->param, 1)) : '';
            $data['url'][$slug->lang] = '/' . $slug->slug . $param;
        }
    ...
    {
        $slug = Slugs::where('module', $module)->where('method', $method)->where('data_id', $data_id)->first();
        $metadata = [];
        if (!empty($slug)) {
            $metadata = [
    ...
    public function createMetadata(Request $request, $data_id = 0)
    {
        ...
        if ($request->hasFile('meta_image')) {
            ...
            if (!empty($request->get('meta_image_old'))) {
                $old_path = str_replace('storage', 'public', $request->get('meta_image_old'));
                Storage::delete($old_path);
            }
        } else {
            if (!empty($request->get('meta_image_del')) && $request->get('meta_image_del') == 1) {
                $old_path = str_replace('storage', 'public', $request->get('meta_image_old'));
                Storage::delete($old_path);
                $attributes['meta_image'] = '';
            }
        }
        $attr_main = array('slug_uid' => $attributes['module'] . '-' . $attributes['method'] . '-' . $attributes['data_id']);
    ...
    public function validatorSlug(Request $request)
    {
        ...
        if (empty($meta['slug'])) {
            $resp = ['error' => 0, 'msg' => 'โปรดระบุ SEO Slug'];
            return $resp;
        }
        $slug = $this->createSlugText($meta['slug']);
        $chk_slug = Slugs::where([['slug', $slug], ['slug_uid', '<>', $meta['module'] . '-' . $meta['method'] . '-' . $request->get('id')]])->first();
        if (!empty($chk_slug)) {
            $resp = ['error' => 1,  'msg' => 'SEO Slug มีการใช้งานแล้ว'];
            return $resp;
        }
    }
    public function validatorSlugUpdate(Request $request)
    {
        ...
        $slug = $this->createSlugText($meta['slug']);
        $chk_slug = Slugs::where([['slug', $slug], ['slug_uid', '<>', $meta['module'] . '-' . $meta['method'] . '-' . $request->get('data_id')]])->first();
