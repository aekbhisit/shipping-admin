...
class MembersExport implements FromCollection, WithHeadings, WithMapping
{
    ...
    public function map($member): array
    {
        return [
            ...
            'unit_show_name'=>(!empty($member->unit_name))?Units::where('unit_no',$member->unit_id)->where('area_id',$member->area_id)->first()->name:'',
    ...
    */
    public function collection()
    {
        ...
        if(!empty($this->filter)){
            ...
            if(!empty(  $this->filter['unit_no'] )){
                $o_member = $o_member->where('unit_id', $this->filter['unit_no'] );
            }
            ...
            if(!empty($this->filter['gender'])){
                if($this->filter['gender']=='2'){
                    $o_member = $o_member->where('gender', 'ชาย'); 
                }

                if($this->filter['gender']=='2'){
                    $o_member = $o_member->where('gender', 'หญิง') ;
                }

                if($request->get('gender')=='3'){
                    $o_member = $o_member->whereNull('gender'); 
                } 

            }

            if(!empty( $this->filter['age_start'])){
                $o_member = $o_member->where('age', '>=',$this->filter['age_start']);
            }
            if(!empty( $this->filter['age_end'])){
                $o_member = $o_member->where('age', '<=',$this->filter['age_end']);
            }


            $o_member = $o_member->with('area')->with('community')->with('create_user')->with('update_user')->withcount('downline')->withcount('samehouse')->withcount('downline_100')->withcount('downline_50')->withcount('downline_20')->withcount('downline_0')  ;

            if(!empty( $this->filter['samehouse_count'])){
                $o_member = $o_member->having('samehouse_count', '>=', $this->filter['samehouse_count']);
            }
            ...
            $o_member = $o_member->with('unit_name') ;
            ...
            $members = Members::with('area')->with('unit_name')->with('community')->with('create_user')->with('update_user')->withcount('downline')->withcount('samehouse')->withcount('downline_100')->withcount('downline_50')->withcount('downline_20')->withcount('downline_0')->all() ;
            // echo '<pre>' ;
            // print_r($members->toArray());
            // exit;
            return $members ;
